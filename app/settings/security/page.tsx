      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '1.5rem 5% 4rem' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: '1.5px solid #E2E8F0', padding: '1.25rem', marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {[
              ...
              <div key={i} style={{ position: 'relative' }}>
                ...
                <input type={show ? 'text' : 'password'} value={f.val} onChange={e => f.set(e.target.value)} placeholder={f.placeholder} style={{ ...inp, paddingRight: '2.5rem' }} onFocus={e => (e.target.style.borderColor = '#0D1B3E')} onBlur={e => (e.target.style.borderColor = '#E5E7EB')} />
                <button onClick={() => setShow(!show)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#9CA3AF', display: 'flex' }}>
          ...
          <button onClick={changePassword} disabled={loading}
            style={{ width: '100%', marginTop: '1rem', padding: '0.8rem', background: '#fff', color: '#111827', border: 'none', borderRadius: 12, fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: '0.875rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>

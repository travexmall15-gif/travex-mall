  ...
  return (
    <main style={{ minHeight:'100vh', background:'var(--sn-page)', paddingTop:108, fontFamily:"'Inter',sans-serif" }}>
      ...
      <div style={{ maxWidth:520, margin:'0 auto', padding:'1.5rem 5% 5rem' }}>
        ...
        {/* Order summary card */}
        <div style={{ background:'var(--sn-bg)', borderRadius:16, padding:'14px 18px', marginBottom:'1.5rem', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <div style={{ fontSize:'0.75rem', color:'var(--sn-subtle)', marginBottom:3 }}>{t('orders.orderTotal')}</div>
            <div style={{ fontSize:'1.2rem', fontWeight:900, color:'var(--sn-text)' }}>TZS {order?.total_amount?.toLocaleString() || '—'}</div>
            <div style={{ fontSize:'0.72rem', color:'var(--sn-subtle)', marginTop:2 }}>{order?.product_name}</div>
          </div>
        ...
        {step === 'option' && (
          <>
            <div style={{ background:'var(--sn-page)', borderRadius:12, padding:'10px 14px', marginBottom:'1rem', display:'flex', alignItems:'center', gap:10 }}>
                ...
                <button key={opt.id} onClick={() => setSelOption(opt.id)}
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                      <span style={{ fontSize:'0.9rem', fontWeight:700, color:'var(--sn-text)' }}>{opt.title}</span>
                    ...
                    <div style={{ fontSize:'0.72rem', color:'var(--sn-subtle)', lineHeight:1.5 }}>{opt.desc}</div>
                  </div>
        ...
        {step === 'location' && (
          <>
            ...
            <p style={{ fontSize:'0.78rem', color:'var(--sn-muted)', marginBottom:'1.25rem', lineHeight:1.5 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:'1rem' }}>
              <div style={{ flex:1, height:1, background:'var(--sn-border)' }} />
              <span style={{ fontSize:'0.7rem', color:'var(--sn-subtle)', fontWeight:600 }}>{t('payment.orTypeAddress')}</span>
              <div style={{ flex:1, height:1, background:'var(--sn-border)' }} />
            </div>
            ...
            <div style={{ background:'var(--sn-bg)', borderRadius:16, border:'1.5px solid #E2E8F0', overflow:'hidden', marginBottom:'1.25rem' }}>
              {[
                ...
                <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', padding:'12px 16px', borderBottom: i<4?'1px solid #F1F5F9':'none', gap:12 }}>
                  <span style={{ fontSize:'0.78rem', color:'var(--sn-subtle)', fontWeight:600, flexShrink:0 }}>{row.label}</span>
                  <span style={{ fontSize:'0.82rem', color:'var(--sn-text)', fontWeight:600, textAlign:'right' }}>{row.value}</span>
                </div>
              ))}
            </div>
        ...
        {step === 'done' && (
          <div style={{ textAlign:'center', padding:'2rem 0' }}>
            ...
            <p style={{ fontSize:'0.85rem', color:'var(--sn-muted)', lineHeight:1.6, marginBottom:'2rem' }}>
              {selOption==='full'

...
function descTool(name:string,category:string,features:string,price:number,shopName:string){
  ...
  const eng = `${name} — now available at ${shopName} on Travex Mall! ${features?`Key features: ${features}. `:''}${ps?`Priced at only ${ps}. `:''}Order now and get fast delivery across Tanzania. Quality guaranteed by a verified Travex seller.`
  const swa = `${name} — inapatikana sasa hivi katika ${shopName} kwenye Travex Mall! ${features?`Maelezo: ${features}. `:''}${ps?`Bei yake ni ${ps} tu. `:''}Agiza sasa na kupata delivery haraka Tanzania nzima.`
...
function coachTool(q:string,ctx:any){
  ...
  if(/(delivery|logistics|transport|shipping)/i.test(q)) return `Use Travex Move for deliveries — connects you with verified bodaboda riders and drivers. Set clear timeframes (Dar: same day, other regions: 1-3 days). Customers appreciate predictability.`
  if(/(review|rating|feedback)/i.test(q)) return `To get good reviews: (1) Deliver on time. (2) Package products properly. (3) Follow up after delivery. (4) Resolve complaints immediately. Good reviews improve your search ranking on Travex Mall.`

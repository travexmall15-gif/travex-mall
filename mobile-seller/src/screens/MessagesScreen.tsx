...
export default function MessagesScreen() {
  ...
  useEffect(() => {
    AsyncStorage.getItem('seller_session').then(raw => {
      ...
      sb.from('pending_payments').select('shop_whatsapp').eq('id', id).maybeSingle().then(({ data: shop }) => {
        if (shop?.shop_whatsapp) setShopWA(shop.shop_whatsapp.replace(/[^0-9]/g, ''))
      })
      sb.from('orders').select('*').eq('shop_id', id).order('created_at', { ascending: false }).then(({ data: orders }) => {
  ...
  if (contacts.length === 0) return (
    <View style={{ flex: 1, backgroundColor: OFF, alignItems: 'center', justifyContent: 'center', padding: 32 }}>
      ...
      <Text style={{ fontSize: 13, color: GRAY, textAlign: 'center', lineHeight: 20 }}>
        Customers who place orders will appear here. You can chat with them on WhatsApp.
      </Text>
    </View>
  )
  ...
  return (
    <FlatList
      ...
      renderItem={({ item: o }) => (
        <TouchableOpacity style={s.item} onPress={() => openWA(o.customer_phone)} activeOpacity={0.85}>
          <View style={s.av}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 11, color: GRAY }}>
              Order: {o.product_name || ''} · {fmtTZS(o.amount || 0)}
            </Text>
          </View>
          <View style={s.wa}>
            <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700' }}>WA</Text>
          </View>
        </TouchableOpacity>
      )}
    />

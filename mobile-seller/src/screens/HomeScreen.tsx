...
export default function HomeScreen({ navigation }: any) {
  ...
  const onRefresh = async () => { setRef(true); await loadData(); setRef(false) }
  ...
  const weekDays = Array.from({length:7},(_,i) => {
    ...
    return { date: d.toISOString().split('T')[0], label: days[d.getDay()===0?6:d.getDay()-1], today: i===6 }
  })
  ...
  return (
    <ScrollView style={{flex:1,backgroundColor:OFF}} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh}/>}>
      <View style={s.topBar}>
        ...
        <View style={[s.planBadge, {backgroundColor: shop?.plan==='premium' ? 'rgba(201,168,76,0.15)':'rgba(100,116,139,0.1)'}]}>
      ...
      <View style={s.pad}>
        ...
        <View style={s.statsGrid}>
          {[
            ...
            {label:'Pending Orders',val:String(pending),  sub:'need confirm', color:pending>0?RED:NAVY},
          ].map(c=>(
            ...
            {weekDays.map((d,i)=>(
              <View key={d.label} style={{flex:1,alignItems:'center',gap:4}}>
                <View style={{flex:1,width:'100%',justifyContent:'flex-end'}}>
                  <View style={{height: Math.max((weekVals[i]/maxVal)*64,3), borderRadius:4, backgroundColor:d.today?GOLD:'#CBD5E1'}}/>
                </View>
        ...
        <View style={{flexDirection:'row',flexWrap:'wrap',gap:10,marginBottom:16}}>
          {[
            {icon:'📦',label:'Add Product', action:()=>navigation.navigate('Products')},
            {icon:'💰',label:'Record Sale', action:()=>navigation.navigate('Finance',{openAdd:true})},
            {icon:'💬',label:'Messages',    action:()=>navigation.navigate('Messages')},
            {icon:'⭐',label:'View Orders', action:()=>navigation.navigate('Orders')},
          ].map(a=>(
            <TouchableOpacity key={a.label} onPress={a.action} style={s.qaBtn} activeOpacity={0.8}>
        ...
        <View style={s.card}>
          {orders.length===0
            ...
            : orders.slice(0,4).map(o=>(
              <TouchableOpacity key={o.id} style={s.orderItem} onPress={()=>navigation.navigate('Orders',{orderId:o.id})} activeOpacity={0.8}>
                <View style={s.orderAv}><Text style={{fontWeight:'800',color:NAVY,fontSize:13}}>{(o.customer_name||'?').slice(0,2).toUpperCase()}</Text></View>
                <View style={{flex:1}}>
                  <Text style={s.orderName}>{o.customer_name||'Customer'}</Text>
                ...
                <View style={{alignItems:'flex-end'}}>
                  ...
                  <View style={[s.statusChip,{backgroundColor:o.status==='pending'?'#FEF3C7':o.status==='confirmed'?'#D1FAE5':'#F1F5F9'}]}>
...
const s = StyleSheet.create({
  ...
  sectionTitle:{ fontSize:14, fontWeight:'800', color:NAVY, marginBottom:10 },
  qaBtn:     { width:'47%', backgroundColor:'#fff', borderRadius:12, borderWidth:1.5, borderColor:'#E2E8F0', padding:14, alignItems:'center', gap:6 },
  qaLabel:   { fontSize:11, fontWeight:'700', color:NAVY },
  ...
  orderName: { fontSize:13, fontWeight:'700', color:NAVY },
  statusChip:{ paddingHorizontal:8, paddingVertical:2, borderRadius:999, marginTop:3 },
})
